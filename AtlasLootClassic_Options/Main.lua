...
for classIndex, className in ipairs(SortedClassList) do
	...
	classFilterTab[className] = {
		order = classIndex,
		name = (CLASS_NAMES_WITH_COLORS[string.upper(className)] or CLASS_NAMES_WITH_COLORS[className] or className),
		type = "group",
		childGroups = "tab",
		get = function(info) return ClassFilterDB[cName] and ClassFilterDB[cName][info[#info]] end,
		set = function(info, value) if ClassFilterDB[cName] then ClassFilterDB[cName][info[#info]] = value end end,
