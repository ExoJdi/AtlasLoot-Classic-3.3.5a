...
local function CreateFavouriteOptions()
	...
	for i = 3, #iconList do
		...
		local iconPath = type(icon) == "table" and icon[1] or icon
		count = count + 1
		args["icon"..i] = {
            order = count,
            type = "execute",
            name = function(info)
				return FavAddon:GetIcon() == iconPath and "^" or ""
            end,
			image = iconPath,
            imageWidth = 20,
            imageHeight = 20,
...
local function CreateSourceOptions()
    ...
    local srcTypes = (Sources and Sources.GetSourceTypes and Sources:GetSourceTypes()) or {}
    for num, srcName in ipairs(srcTypes) do
        data[tostring(num)] = {
            order = num,
            type = "toggle",
            --width = "full",
