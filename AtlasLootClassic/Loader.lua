...
local function LoaderEvents(frame, event, arg1)
	if event == "ADDON_LOADED" then
		local key = AtlasLoot.NormalizeAddonName and AtlasLoot.NormalizeAddonName(arg1) or arg1
		if ModuleList[key] then
			ModuleList[key].loaded = true
			collectgarbage("collect")
		end
		if LoaderQueue[key] then
			if type(LoaderQueue[key]) == "function" then
				LoaderQueue[key](key)
			elseif type(LoaderQueue[key]) == "table" then
				for i = 1, #LoaderQueue[key] do
					if type(LoaderQueue[key][i]) == "function" then
						LoaderQueue[key][i](key)
					end
				end
			end
			LoaderQueue[key] = nil
...
function Loader.Init()
	...
	for i=1,GetNumAddOns() do
		...
		if name and str_find(name, "AtlasLootClassic_") then
			local folderName = name
			local baseName = AtlasLoot.NormalizeAddonName and AtlasLoot.NormalizeAddonName(folderName) or folderName
			if not ModuleList[baseName] then
				ModuleList[baseName] = {
					folderName = folderName,
					index = i,
					enabled = GetAddOnEnableState(playerName, i) ~= 0,
					loaded = IsAddOnLoaded(folderName),
					loadReason = reason,
					standardModule = ATLASLOOT_MODULE_LIST_NAMES[baseName],
					moduleName = GetAddOnMetadata(folderName, "X-AtlasLootClassic-ModuleName") or baseName,
					lootModule = GetAddOnMetadata(folderName, "X-AtlasLootClassic-LootModule"),
				}
			end
		end
	end
...
function Loader:LoadModule(moduleName, onLoadFunction, oneFunction)
	...
	if InCombatLockdown() then
		...
		LoadAddOn(ModuleList[moduleName].folderName or moduleName)
	end
end

function Loader:IsModuleLoaded(moduleName)
	if not moduleName or not ModuleList[moduleName] then return end
	return IsAddOnLoaded(ModuleList[moduleName].folderName or moduleName)
end
...
function Loader:GetLootModuleList()
	...
	for i = 1, #ATLASLOOT_MODULE_LIST do
		...
		if moduleTable.addonName and ModuleList[moduleTable.addonName] and ModuleList[moduleTable.addonName].lootModule == "1" and ModuleList[moduleTable.addonName].enabled then
			...
			local folderName = ModuleList[moduleTable.addonName].folderName or moduleTable.addonName
			data.module[#data.module+1] = {
				addonName = moduleTable.addonName,
				name = displayName,
				tt_title = moduleTable.tt_title or moduleTable.name,
				tt_text = moduleTable.tt_text or select(3, GetAddOnInfo(folderName)),
			}
		end
	end
	-- custom modules
