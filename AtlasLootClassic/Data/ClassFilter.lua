...
local FILTER_DATA = {
    ...
    itemSubClass = {
        ...
        [2] = {
			[0] 		    = C.HUNTER + C.PALADIN + C.SHAMAN + C.WARRIOR + C.DEATHKNIGHT + C.ROGUE, -- One-Handed Axes
...
function ClassFilter.GetStatListForOptions()
    if not db and AtlasLoot.db then
        db = AtlasLoot.db.ClassFilter
    end
    ...
    return STAT_LIST, OptionsClassSort, db or {}
end
...
function ClassFilter.ClassCanUseItem(className, itemID)
    ...
    if ITEM_SUB_CLASS_IGNORE[itemClassID] and ITEM_SUB_CLASS_IGNORE[itemClassID][itemEquipLoc] then
        -- ignore
    elseif CLASS_FILTER.itemSubClass[itemClassID] and CLASS_FILTER.itemSubClass[itemClassID][itemSubClassID] and not CLASS_FILTER.itemSubClass[itemClassID][itemSubClassID][classID] then
        return false
    end
