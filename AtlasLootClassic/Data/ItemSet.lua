...
local SetNameCache = {}
local SetNameTooltip

local function GetSetNameFromTooltip(itemID)
	if not itemID then return end
	if SetNameCache[itemID] ~= nil then
		return SetNameCache[itemID]
	end
	if not SetNameTooltip then
		SetNameTooltip = CreateFrame("GameTooltip", "AtlasLootClassicItemSetNameTooltip", nil, "GameTooltipTemplate")
		SetNameTooltip:SetOwner(UIParent, "ANCHOR_NONE")
	end
	SetNameTooltip:ClearLines()
	-- 3.3.5 does not reliably support SetItemByID on GameTooltip
	SetNameTooltip:SetHyperlink("item:"..itemID)
	local name
	local retrieving
	for i = 2, 20 do
		local line = _G["AtlasLootClassicItemSetNameTooltipTextLeft"..i]
		local t = line and line:GetText()
		if t then
			-- strip color codes and atlas tags
			t = t:gsub("%|c%x%x%x%x%x%x%x%x", ""):gsub("%|r", "")
			t = t:gsub("%|A:[^%|]*%|a", "")
			if t:find("Retrieving item information", 1, true) then
				retrieving = true
			end
			local n = t:match("^(.+)%s%(%d+/%d+%)$")
			if n and n ~= "" then
				name = n
				break
			end
	...
	-- Do not cache negative results while the client is still retrieving item info.
	if name then
		SetNameCache[itemID] = name
	else
		-- Never cache negative results; item info can arrive later.
		SetNameCache[itemID] = nil
	end
	return name
end
...
function ItemSet.GetSetName(setID, addQualityColor)
	...
	if not setName or setName == UNKNOWN then
		local firstItem = ITEM_SET_DATA[setID] and ITEM_SET_DATA[setID][1] and ITEM_SET_DATA[setID][1][1]
		setName = firstItem and GetSetNameFromTooltip(firstItem) or nil
	end
	setName = setName or UNKNOWN
	if ITEM_SET_DATA[setID][7] then
