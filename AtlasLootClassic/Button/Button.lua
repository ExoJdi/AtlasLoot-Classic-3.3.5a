...
_G.LOOT_BORDER_BY_QUALITY_AL = _G.LOOT_BORDER_BY_QUALITY_AL or {
	[0] = "Interface\\Buttons\\UI-ActionButton-Border",
	[1] = "Interface\\Buttons\\UI-ActionButton-Border",
	[2] = "Interface\\Buttons\\UI-ActionButton-Border",
	[3] = "Interface\\Buttons\\UI-ActionButton-Border",
	[4] = "Interface\\Buttons\\UI-ActionButton-Border",
	[5] = "Interface\\Buttons\\UI-ActionButton-Border",
}

	button_types = button_types or {}
	button_types_index = button_types_index or {}
	extra_button_types = extra_button_types or {}
	extra_button_types_index = extra_button_types_index or {}
	BUTTON_COUNT = BUTTON_COUNT or 0
	SEC_BUTTON_COUNT = SEC_BUTTON_COUNT or 0
local Button = {}
local Proto = {}
local API = {}
AtlasLoot.Button = Button

-- Chat link helper (3.3.5a). Some button types expect this API.
function Button:AddChatLink(link)
	if not link or link == "" then return end
	if _G.ChatEdit_InsertLink then
		_G.ChatEdit_InsertLink(link)
	elseif _G.ChatFrameEditBox and _G.ChatFrameEditBox:IsShown() then
		_G.ChatFrameEditBox:Insert(link)
	end
end


-- ===== 3.3.5a required public helpers (used by ItemDB + Options) =====
...
local PLAYER_FACTION_ID = (UnitFactionGroup("player") == "Horde") and 0 or 1
function Button:GetWoWHeadLocale()
  return WOW_HEAD_LOCALE[GetLocale()] or "www"
end
...
local STANDART_FORMAT_TABLE = {
	"Item",
	"Item",
}

for i = 1, #STANDART_TABLE do
	STANDART_FORMAT_TABLE[#STANDART_FORMAT_TABLE+1] = STANDART_TABLE[i]
end

function Button:CreateFormatTable(tab)
	tab = tab or {}
	for i = 1, #STANDART_TABLE do
		tab[#tab+1] = STANDART_TABLE[i]
	end
	return tab
end
Button.Proto = Proto
...
local function SetAtlasSafe(textureObj, atlasName, fallbackTexture)
  if not textureObj then
    return
  end
  if atlasName and textureObj.SetAtlas then
    local ok = pcall(textureObj.SetAtlas, textureObj, atlasName)
    if ok then
      return
    end
  end
  if atlasName and ALPrivate and ALPrivate.ICONS_PATH then
    textureObj:SetTexture(ALPrivate.ICONS_PATH .. atlasName)
    return
  end
...
local function Button_SetNormalTexture(self, texture)
	-- Keep the real button NormalTexture cleared without touching the icon texture.
	if texture == nil then
		if self.OriSetNormalTexture then
			self:OriSetNormalTexture(nil)
		end
		return
	end
	-- Prevent default button backdrops (blue quickslot squares etc.) from overwriting item icons.
	if type(texture) == "string" and (texture:find("UI%-Quickslot") or texture:find("UI%-ActionButton")) then
		-- Ignore these textures completely.
		return
	end
	if self.icon then
		if type(texture) == "string" and texture:lower():find("interface\\icons\\classicon_", 1, true) then
			local c = texture:lower():match("classicon_([%a]+)")
			if c then
				c = c:upper()
				local map = ALPrivate and ALPrivate.CLASS_ICON_PATH
...
local function Button_ForceSetText(self, text, force)
	if type(text) == "string" then
		-- remove broken color tags like "|c" without a 8-hex color code
		text = text:gsub("%|c(%x%x%x%x%x%x%x%x)", "\001%1")
		text = text:gsub("%|c", "")
		text = text:gsub("\001(%x%x%x%x%x%x%x%x)", "|c%1")
		text = text:gsub("%|A:[^%|]*%|a", "")
	end
...
local function Button_Overlay_SetQualityBorder(self, qualityID)
  -- LootToast-style border (ModernUI). This texture exists in the addon and
  -- should sit slightly outside the icon so it doesn't cover it.
  self:ClearAllPoints()
  local icon = self.icon
  if icon then
    local p = 1
    self:SetPoint("TOPLEFT", icon, "TOPLEFT", -p, p)
    self:SetPoint("BOTTOMRIGHT", icon, "BOTTOMRIGHT", p, -p)
  end
  self:SetTexture("Interface\\AddOns\\AtlasLootClassic\\Images\\loottoast-itemborder-white")
  -- Some icon textures (especially for token/popup/secondary buttons) can end up
  -- with higher sublevels. Force the border to the very top.
  self:SetDrawLayer("OVERLAY", 10)
  self:SetTexCoord(0, 1, 0, 1)
  self:SetBlendMode("BLEND")
  self:SetAlpha(1)
  if qualityID == "gold" then
    self:SetVertexColor(1, 0.82, 0)
  elseif type(qualityID) == "number" and GetItemQualityColor then
    local r, g, b = GetItemQualityColor(qualityID)
...
local function Button_Overlay_SetAchievementBorder(self, set, isGuildAc)
	...
	-- Use the same LootToast border as items to keep consistent sizing across all icon buttons.
	self:ClearAllPoints()
	local icon = self.icon
	if icon then
		local p = 1
	...
	self:SetTexCoord(0, 1, 0, 1)
	self:SetBlendMode("BLEND")
	-- Guild achievements use a green-ish tone in the default UI.
	if isGuildAc then
		self:SetVertexColor(0.1, 1.0, 0.1)
	else
		self:SetVertexColor(1, 1, 1)
	end
	self:SetAlpha(1)
...
function Button:Create()
	...
	button.overlay:SetDrawLayer("OVERLAY", 10)
	...
	SetAtlasSafe(button.favourite, "VignetteKill", "Interface\\COMMON\\ReputationStar")
	...
	button.secButton.icon:SetTexCoord(0.07, 0.93, 0.07, 0.93)
	...
	button.secButton.overlay:SetDrawLayer("OVERLAY", 10)
	...
	SetAtlasSafe(button.secButton.favourite, "VignetteKill", "Interface\\COMMON\\ReputationStar")
	...
	if self.factionIcon then
		if tab[ATLASLOOT_IT_HORDE] or tab[ATLASLOOT_IT_ALLIANCE] then
			...
			if horde or alliance then
                ...
                local factionAtlas = FACTION_TEXTURES[ ( horde and alliance ) and PLAYER_FACTION_ID or ( horde and 0 or 1 ) ]
                local ok = false
                if factionAtlas and self.factionIcon.SetAtlas and C_Texture and C_Texture.GetAtlasInfo then
                    ok = pcall(self.factionIcon.SetAtlas, self.factionIcon, factionAtlas, true)
                end
                if not ok then
                    self.factionIcon:SetTexture("Interface\\Glues\\CharacterCreate\\UI-CharacterCreate-Factions")
                end
				end
			end
...
function Button:GetExtraType(typ)
		assert(extra_button_types[typ], typ.." type not found")
		return extra_button_types[typ]
end
