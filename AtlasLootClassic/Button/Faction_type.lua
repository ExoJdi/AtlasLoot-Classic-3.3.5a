...
local FACTION_IMAGES = {
	...
	-- Wrath
	[1037] = "Interface\\Icons\\INV_BannerPVP_02", -- Alliance Vanguard
}
...
function AtlasLoot:Faction_GetFactionName(factionID)
	if factionID == 91106 or factionID == 911006 then
		factionID = 1106
	end
...
function Faction.ShowToolTipFrame(button)
	...
	local nameW
	if frame.name.GetUnboundedStringWidth then
		nameW = frame.name:GetUnboundedStringWidth()
	else
		nameW = frame.name:GetStringWidth()
	end
	local newWidth = 30 + (nameW or 0)
