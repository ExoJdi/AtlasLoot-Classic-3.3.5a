...
local function UpdateFrames(noPageUpdate, forceContentUpdate)
    ...
    -- SOUNDS
    local soundFrame = GUI.SoundFrame
    if soundFrame and moduleData[dataID].items[bossID].npcID and SoundData:GetNpcData(moduleData[dataID].items[bossID].npcID) then
        soundFrame.npcID = moduleData[dataID].items[bossID].npcID
        contentFrame.soundsButton:Show()
    else
        if soundFrame then
            soundFrame.npcID = nil
        end
        contentFrame.soundsButton:Hide()
        if soundFrame and contentFrame.shownFrame == soundFrame.frame then
            contentFrame.shownFrame = nil
            if soundFrame.frame then
...
local function SoundButtonOnClick(self)
    if GUI.SoundFrame and GUI.SoundFrame.Show then
        GUI.SoundFrame:Show()
        UpdateFrames(true)
    end
end
...
function GUI:Toggle()
    if not self.frame and self.Create then
        self:Create()
    end
...
function GUI:Create()
    ...
    frame.titleFrame.infoButton = CreateFrame("Button", nil, frame)
    frame.titleFrame.infoButton:SetSize(16, 16)
    frame.titleFrame.infoButton:SetNormalTexture("Interface\\GossipFrame\\InfoIcon")
    frame.titleFrame.infoButton:SetHighlightTexture("Interface\\Buttons\\UI-Common-MouseHilight", "ADD")
    ...
    local function CreateBorderTexture()
        local t = frame.gameVersionButton:CreateTexture(nil, "ARTWORK")
        return t
    end
    ...
    local top = CreateBorderTexture()
    top:SetPoint("TOPLEFT", -lineGap, lineGap)
    top:SetPoint("TOPRIGHT", lineGap, lineGap)
    top:SetHeight(thickness)

    local bottom = CreateBorderTexture()
    bottom:SetPoint("BOTTOMLEFT", -lineGap, -lineGap)
    bottom:SetPoint("BOTTOMRIGHT", lineGap, -lineGap)
    bottom:SetHeight(thickness)

    local left = CreateBorderTexture()
    left:SetPoint("TOPLEFT", -lineGap, lineGap)
    left:SetPoint("BOTTOMLEFT", -lineGap, -lineGap)
    left:SetWidth(thickness)

    local right = CreateBorderTexture()
    right:SetPoint("TOPRIGHT", lineGap, lineGap)
    right:SetPoint("BOTTOMRIGHT", lineGap, -lineGap)
...
function GUI.RefreshContentBackGround()
    ...
    if db.contentBottomBar.useContentColor and ( frame.downBG.curAlpha ~= db.contentBottomBar.bgColor.a or frame.downBG.curColor ~= GUI.curBgInfo[1]) then
        frame.downBG:SetColorTexture(GUI.curBgInfo[1][1], GUI.curBgInfo[1][2], GUI.curBgInfo[1][3], db.contentBottomBar.bgColor.a)
