...
function ItemFrame.UpdateFilterItem(buttonID, reset)
    ...
    if type(buttonID) == "number" then
        button = ItemFrame.frame.ItemButtons and ItemFrame.frame.ItemButtons[buttonID] or nil
    else
        button = buttonID
    end
    if not button or type(button) == "number" then return reset end
    ...
    if ItemFrame.SearchString then
		local text = button.RawName or (button.name and button.name:GetText())
...
function ItemFrame.UpdateFilter()
    ...
    if reset then
		for i=1,30 do
			local b = ItemFrame.frame.ItemButtons and ItemFrame.frame.ItemButtons[i] or nil
...
function ItemFrame:Refresh(skipProtect)
	...
	if not AtlasLoot.db.GUI.selected[1] or not AtlasLoot.db.GUI.selected[2] or not AtlasLoot.db.GUI.selected[3] then
		return
	end
