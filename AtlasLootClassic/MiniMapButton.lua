...
local MiniMapLDB = LibStub:GetLibrary("LibDataBroker-1.1"):NewDataObject("AtlasLoot", {
	...
	OnClick = function(self, button)
		if button == "RightButton" then
			local fav = AtlasLoot.Addons and AtlasLoot.Addons.GetAddon and AtlasLoot.Addons:GetAddon("Favourites")
			if fav and fav.GUI and fav.GUI.Toggle then
				fav.GUI:Toggle()
			else
				-- fallback: open main window if favourites is not available
				if AtlasLoot and AtlasLoot.GUI and AtlasLoot.GUI.Toggle then AtlasLoot.GUI:Toggle() end
			end
...
function MiniMapButton.Lock_Toggle()
    if AtlasLoot.db.minimap.locked then
        AtlasLoot.db.minimap.locked = false
    else
        AtlasLoot.db.minimap.locked = true
    end
    if ALButton and ALButton.Refresh then
        ALButton:Refresh("AtlasLoot")
    end
end
