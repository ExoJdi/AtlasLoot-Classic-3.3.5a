...
function Tooltip.GetTooltip()
	local tt = AtlasLoot.db.Tooltip.useGameTooltip and _G["GameTooltip"] or ( _G[AtlasLoot.db.Tooltip.tooltip or STANDART_TOOLTIP] or AtlasLootTooltip )
	if tt and not tt.Reset then
		tt.Reset = function(self)
			self:ClearLines()
		end
	end
	return tt
...
local function OnTooltipSetItem_Hook(self)
	if (self.IsForbidden and self:IsForbidden()) or not AtlasLoot.db.showTooltipInfoGlobal then return end
...
local PLAYER_GUID_REGISTER = {
	...
	["Player-4385-05E5AD51"] = format("|T523897:0|t "..COLOR, "AtlasLoot Maintainer"), -- Snowflame
	["Player-4385-05E59BF7"] = format("|T523897:0|t "..COLOR, "AtlasLoot Maintainer"), -- Snowflame Druid
}
