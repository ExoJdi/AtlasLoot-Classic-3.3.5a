...
function Proto:Get(mouseButton)
	...
	local defaultsTypes = self.defaults and self.defaults.types
	local dbTypes = db and db.types
	local function IsTypeEnabled(key)
		if not key then return false end
		if not dbTypes then
			return defaultsTypes and defaultsTypes[key] and true or false
		end
		local v = rawget(dbTypes, key)
		if v == nil then
			return defaultsTypes and defaultsTypes[key] and true or false
		end
		...
		if IsModifierKeyDown() then
			if IsShiftKeyDown() then
				if IsLeftShiftKeyDown() and handler.LeftShift then
					return IsTypeEnabled(handler.LeftShift) and handler.LeftShift or nil
				elseif IsRightShiftKeyDown() and handler.RightShift then
					return IsTypeEnabled(handler.RightShift) and handler.RightShift or nil
				end
				if handler.Shift then
					return IsTypeEnabled(handler.Shift) and handler.Shift or nil
				end
			elseif IsAltKeyDown() then
				if IsLeftAltKeyDown() and handler.LeftAlt then
					return IsTypeEnabled(handler.LeftAlt) and handler.LeftAlt or nil
				elseif IsRightAltKeyDown() and handler.RightAlt then
					return IsTypeEnabled(handler.RightAlt) and handler.RightAlt or nil
				end
				if handler.Alt then
					return IsTypeEnabled(handler.Alt) and handler.Alt or nil
				end
			elseif IsControlKeyDown() then
				if IsLeftControlKeyDown() and handler.LeftCtrl then
					return IsTypeEnabled(handler.LeftCtrl) and handler.LeftCtrl or nil
				elseif IsRightControlKeyDown() and handler.RightCtrl then
					return IsTypeEnabled(handler.RightCtrl) and handler.RightCtrl or nil
				end
				if handler.Ctrl then
					return IsTypeEnabled(handler.Ctrl) and handler.Ctrl or nil
				end
			end
		end
		return IsTypeEnabled(handler.None) and handler.None or nil
	end
end
