...
if not AtlasLoot.IngameLocales then
	AtlasLoot.IngameLocales = setmetatable({}, {__index = function(_, k)
		if AtlasLoot.Locales and AtlasLoot.Locales[k] then return AtlasLoot.Locales[k] end
		return k
	end})
end
...
function AtlasLoot:OnInitialize()
	...
	if not self.db.GUI.selectedGameVersion then
		self.db.GUI.selectedGameVersion = self:GetGameVersion()
	elseif self:GameVersion_LT(self.db.GUI.selectedGameVersion) then
		self.db.GUI.selectedGameVersion = self:GetGameVersion()
	end
	if self.IS_WRATH and self.db.GUI.selectedGameVersion ~= self.WRATH_VERSION_NUM then
		self.db.GUI.selectedGameVersion = self.WRATH_VERSION_NUM
	end
	if self.db.GUI.selected and self.db.GUI.selected[2] == "BlackrockCaverns" then
		self.db.GUI.selected[2] = "UtgardeKeep"
	end
...
function AtlasLoot:GetColoredClassNames()
	if not CLASS_NAMES_WITH_COLORS then
		...
		for k,v in pairs(RAID_CLASS_COLORS) do
			if v.colorStr then
				local n = (AtlasLoot.IngameLocales and AtlasLoot.IngameLocales[k]) or (LOCALIZED_CLASS_NAMES_MALE and LOCALIZED_CLASS_NAMES_MALE[k]) or k
				CLASS_NAMES_WITH_COLORS[k] = format(CLASS_COLOR_FORMAT, v.colorStr, n)
			end
		end
	end
