...
local GetItemInfoInstant = _G.GetItemInfoInstant or function(item)
	local id = tonumber(tostring(item):match("item:(%d+)") or tostring(item):match("^(%d+)$"))
	return id
end
local GetAddOnInfo = _G.GetAddOnInfo
local GetServerTime = _G.GetServerTime
local function ItemExist(itemID)
	return itemID and GetItemInfo(itemID) ~= nil
end
...
local EXTRA_ICONS = {
    "Interface\\Icons\\Spell_Deathknight_ClassIcon", -- Death Knight
    ...
    "Interface\\Icons\\inv_misc_monsterclaw_04", -- Druid
    ...
    "Interface\\Icons\\Ability_ThunderBolt", -- Paladin
    ...
    "Interface\\Icons\\Spell_Nature_BloodLust", -- Shaman
    ...
    "Interface\\Icons\\Spell_Nature_Drowsy", -- Warlock
    ...
    "Interface\\Icons\\INV_Sword_27", -- Warrior
...
local function OnTooltipSetItem_Hook(self)
    if (self.IsForbidden and self:IsForbidden()) or not Favourites.db.enabled or (not Favourites.db.showIconInTT and not Favourites.db.showListInTT) then return end
...
function Favourites:UpdateDb()
    ...
    if self.GUI and self.GUI.ItemListUpdate then
...
function Favourites.OnInitialize()
    Favourites:UpdateDb()
    if Favourites.GUI and Favourites.GUI.OnInitialize then
...
function Favourites:InsertChatLink()
    ...
    local link = ("|Hgarrmission:atlaslootclassic_fav|h|cFF8800FF[%s |r|cFF8800FF- %s]|h|r"):format(characterName, displayName)

    local editbox = GetCurrentKeyBoardFocus()
    if not editbox and ChatEdit_ChooseBoxForSend and ChatEdit_ActivateChat then
        editbox = ChatEdit_ChooseBoxForSend()
        if editbox then ChatEdit_ActivateChat(editbox) end
    end
    if editbox then
        editbox:Insert(link)
    end
...
function Favourites:GetFavouriteItemText(itemId, listId)
	local listData = (self.db and self.db.lists and self.db.lists[listId]) or (self.globalDb and self.globalDb.lists and self.globalDb.lists[listId]) or {}
            ...
            if strsub(icon, 1, 1) == ATLAS_ICON_IDENTIFIER then
                if icon and icon ~= texture:GetAtlas() then
							...
							if texture.SetAtlas and C_Texture and C_Texture.GetAtlasInfo then
								ok = pcall(texture.SetAtlas, texture, strsub(icon, 2), true)
							end
							if not ok then
								texture:SetTexture(STD_ICON)
							end
                end
            else
                texture:SetTexture(icon)
...
function Favourites:ExportItemList(listID, isGlobalList)
    ...
    for entry in pairs(list) do
        if strsub(entry, 1, 2) ~= "__" then
            ...
            if entryId then
                local exportString = format(EXPORT_PATTERN, "i", entryId, list.notes and list.notes[entry] or "")
