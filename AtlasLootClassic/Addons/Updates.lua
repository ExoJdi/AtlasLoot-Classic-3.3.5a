...
--- Update Text
---
local update_version = 6.0
local update_text = {
    "Welcome to AtlasLootClassic |cFF33FFBDWotLK|r v6.0.0!",
    "Fixes to holidays, thanks to all the contributors on GitHub!",
    "|cFFFF8000HELP|r: If you find any issues or something missing, let me know on GitHub ( |cFF3366CChttps://github.com/ExoJdi/AtlasLootClassic_WotLK_3.3.5a/issues|r )",
    "Again, if you appreciate all the work, feel free to support me at the link below (click, CTRL+A, and CTRL+C to copy)"
}
local donation_link = "https://boosty.to/exojdi"
...
function Updates:Open()
    if not Updates.GUI then
		...
		if frame.content.editBox1.Disable then
			frame.content.editBox1:Disable()
		else
			frame.content.editBox1:EnableMouse(false)
			frame.content.editBox1:SetScript("OnEditFocusGained", frame.content.editBox1.ClearFocus)
		end
		...
		if frame.content.editBox2.Disable then
			frame.content.editBox2:Disable()
		else
			frame.content.editBox2:EnableMouse(false)
			frame.content.editBox2:SetScript("OnEditFocusGained", frame.content.editBox2.ClearFocus)
		end
