...
local EMPTY_SLOTS = {
    [INVSLOT_AMMO]      = "Interface\\PaperDoll\\UI-PaperDoll-Slot-Ammo",
    ...
    [INVSLOT_BACK]      = "Interface\\PaperDoll\\UI-PaperDoll-Slot-Back",
    ...
    [20] = "Interface\\PaperDoll\\UI-PaperDoll-Slot-Bag",
    [21] = "Interface\\PaperDoll\\UI-PaperDoll-Slot-Bag",
    [22] = "Interface\\PaperDoll\\UI-PaperDoll-Slot-Bag",
    [23] = "Interface\\PaperDoll\\UI-PaperDoll-Slot-Bag",
}
...
local function GUI_FrameOnHide(self)
	if GUI and GUI.frame and GUI.frame.content and GUI.frame.content.slotFrame then
...
local function SlotButton_SetSlotItem(self, itemID)
    if itemID and itemID ~= true and ItemExist(itemID) then
        ...
        self.overlay:Hide()
        if self.modelFrame then
			if self.modelFrame.UndressSlot then
				self.modelFrame:UndressSlot(self.slotID)
			elseif self.modelFrame.Undress then
				self.modelFrame:Undress()
			elseif UndressSlot then
				UndressSlot(self.slotID)
			end
        end
        self:UnregisterEvent("GET_ITEM_INFO_RECEIVED")
        self.ItemID = nil
...
local function Slot_CreateSlotFrame(frame)
    ...
    frame.modelFrame:SetScript("OnShow", function(self) self:SetUnit("player") end)
...
local function ItemScroll_SetItems(self, itemList, forceUpdate)
    ...
    self.curPos = 1
    local rows = ceil(#itemList / self.maxItemsPerRow)
    self.maxScroll = rows - self.maxItemRows + 1
...
function GUI:Create()
    if not self.frame then
		...
		frame.infoButton = CreateFrame("Button", frameName.."-InfoButton", frame, "UIPanelInfoButton")
