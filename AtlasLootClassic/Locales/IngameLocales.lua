...
local function CleanAuctionCategoryString(v, fallback)
	if type(v) == "string" and not v:match("^AUCTION_CATEGORY") then
		return v
	end
	return fallback
end
...
local GetDifficultyInfo = _G.GetDifficultyInfo or function() return "" end
...
local GetMapNameByID = function() return "" end
if _G.C_Map and _G.C_Map.GetMapInfo then
  GetMapNameByID = function(id) local info = _G.C_Map.GetMapInfo(id); return info and info.name or "" end
end
...
local function GetCurrencyName(currencyID)
	if type(GetCurrencyInfo) == "function" then
		local name = GetCurrencyInfo(currencyID)
...
local IngameLocales = {
	...
	["Jewelcrafting"] = GetSpellName(25229) or UNKNOWN,
	...
	-- glyphs
	["Minor Glyph"] = GLOBAL["Minor Glyph"],
	["Minor Glyphs"] = GLOBAL["Minor Glyphs"],
	["Major Glyph"] = GLOBAL["Major Glyph"],
	["Major Glyphs"] = GLOBAL["Major Glyphs"],
	...
	["Parts"] = GetItemSubClassInfo(7,1),
		["Projectile"] = (GetItemClassInfo and GetItemClassInfo(6)) or (select(6, GetAuctionItemClasses())),
	...
	["Gems"]            = CleanAuctionCategoryString((GLOBAL["AUCTION_CATEGORY_GEMS"] or _G.AUCTION_CATEGORY_GEMS or _G.GEMS), "Gems"),
